        ...
        <div className="relative">
            ...
            <div className="flex items-center pb-3 px-6 py-4" style={hoveredOnDelete ? hoveredOnDeleteStyle : null}>
                ...
                <div className="text-gray-700 px-4 text-lg">
                    ...
                    <div className="text-base">Quantity: <span>{item.quantity}</span></div>
                </div>
                {/* item price & controls*/}
                <div className="flex flex-col items-center justify-center ml-auto">
                    <div className="text-yellow-600 text-lg mb-4">$ {price}</div>
                    <div className="text-gray-100 text-sm flex">
                        <button className="focus:outline-none border-gray-800 w-5 h-5 flex justify-center items-center border bg-gray-800 btn" onClick={()=> dispatch(decrementQuantity(index))} disabled={item.quantity === 1} >-</button>
                        <button className="focus:outline-none border-gray-800 w-5 h-5 flex justify-center items-center border bg-black btn" onClick={()=> dispatch(incrementQuantity(index))}>+</button>
                    </div>
                </div>
            </div>
        </div>
